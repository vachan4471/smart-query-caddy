  ...
  return (
    <div>
      <h2 className={`text-xl font-semibold mb-4 flex items-center ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
        {loading ? (
          <>
            <BrainCircuitIcon className={`animate-pulse mr-2 ${isDarkMode ? 'text-blue-400' : 'text-blue-500'}`} size={20} />
            TDS AI Assistant
          </>
        ) : result ? (
          <>
            <CheckCircleIcon className={`${isDarkMode ? 'text-green-400' : 'text-green-500'} mr-2`} size={20} />
            Generated Answer
          </>
        ) : null}
      </h2>
      ...
      {loading ? (
        <div className={`${isDarkMode ? 'bg-slate-700/30' : 'bg-slate-100/80'} rounded-lg p-6 flex flex-col items-center justify-center min-h-[200px]`}>
          <div className="text-center space-y-3">
            <div className="relative w-16 h-16 mb-4">
              <div className={`absolute inset-0 rounded-full border-t-2 border-b-2 ${isDarkMode ? 'border-blue-500' : 'border-blue-400'} animate-spin`}></div>
              <div className={`absolute inset-2 rounded-full border-t-2 border-b-2 ${isDarkMode ? 'border-purple-500' : 'border-purple-400'} animate-spin animate-delay-150`}></div>
              <div className={`absolute inset-4 rounded-full border-t-2 border-b-2 ${isDarkMode ? 'border-pink-500' : 'border-pink-400'} animate-spin animate-delay-300`}></div>
              <CpuIcon className={`absolute inset-0 m-auto ${isDarkMode ? 'text-blue-400' : 'text-blue-500'} animate-pulse`} size={24} />
            </div>
            ...
            <div className="flex justify-center gap-1 mt-2">
              ...
              <div className={`w-2 h-2 ${isDarkMode ? 'bg-purple-400' : 'bg-purple-500'} rounded-full animate-bounce delay-100`}></div>
              <div className={`w-2 h-2 ${isDarkMode ? 'bg-pink-400' : 'bg-pink-500'} rounded-full animate-bounce delay-200`}></div>
            </div>
        ...
        <Card className={`${
          ...
          <CardContent className="p-6">
            <div className={`rounded ${
              ...
              {isTyping && (
                <span className={`inline-block w-2 h-4 ${isDarkMode ? 'bg-blue-400' : 'bg-blue-500'} ml-1 animate-pulse absolute`}></span>
              )}
            </div>
          </CardContent>
          <CardFooter className={`${
            isDarkMode 
              ...
              : 'bg-slate-100/50 border-t border-slate-200'
          } px-6 py-3 flex justify-between items-center`}>
            <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Answer ready for submission</span>
            <Button 
              variant="outline" 
              size="sm" 
              ...
              className={`${
                isDarkMode 
                  ...
                  : 'bg-transparent border-slate-300 text-slate-700 hover:bg-slate-200 hover:text-slate-900'
              }`}
            >
